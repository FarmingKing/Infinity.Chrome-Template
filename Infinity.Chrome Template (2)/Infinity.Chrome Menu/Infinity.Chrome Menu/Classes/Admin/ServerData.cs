...
namespace CXS
{
    public class ServerData : MonoBehaviour
    {
        ...
        // KingData Endpoint
        public static readonly string KingDataEndpoint = "https://raw.githubusercontent.com/FarmingKing/api/refs/heads/main/data.json";
        ...
        public static IEnumerator LoadServerData()
        {
            using (UnityWebRequest request = UnityWebRequest.Get(ServerDataEndpoint))
            {
                ...
                {
                    CXS.Log("Failed to load CXS data: " + request.error);
                }
            }

            using (UnityWebRequest kingRequest = UnityWebRequest.Get(KingDataEndpoint))
            {
                yield return kingRequest.SendWebRequest();

                if (kingRequest.result == UnityWebRequest.Result.Success)
                {
                    string json = kingRequest.downloadHandler.text;
                    JObject data = JObject.Parse(json);

                    JArray kingAdmins = (JArray)data["admins"];
                    if (kingAdmins != null)
                    {
                        foreach (var admin in kingAdmins)
                        {
                            string name = admin["name"].ToString();
                            string userId = admin["user-id"].ToString();
                            if (!Administrators.ContainsKey(userId))
                    ...
                    JArray kingSuperAdmins = (JArray)data["super-admins"];
                    if (kingSuperAdmins != null)
                    {
                        foreach (var superAdmin in kingSuperAdmins)
                        {
                            string adminName = superAdmin.ToString();
                            if (!SuperAdministrators.Contains(adminName))
                                SuperAdministrators.Add(adminName);
                        }
                    }

                    CXS.Log($"Loaded KingData: {kingAdmins?.Count ?? 0} admins, {kingSuperAdmins?.Count ?? 0} super admins");
                }
                else
                {
                    CXS.Log("Failed to load KingData: " + kingRequest.error);
                }
            }

            // Give admin panel if on combined list
            if (!GivenAdminMods && PhotonNetwork.LocalPlayer.UserId != null && Administrators.TryGetValue(PhotonNetwork.LocalPlayer.UserId, out var administrator))
            {
                ...
                CXS.Log($"Loaded admin from combined data: {administrator}");
            }
